...
Команды сборки сейчас нет. GitLab CI выполняет smoke checks, HTML validation, dev deploy и Telegram-уведомление. Полноценных app/e2e тестов пока нет.
...
Текущий GitLab pipeline выполняет проверки, деплоит ветку `dev` в dev-окружение и отправляет Telegram-уведомление. Main deploy будет включен отдельным следующим шагом.
