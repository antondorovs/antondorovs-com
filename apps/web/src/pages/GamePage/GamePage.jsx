...
import { GameSession } from '../../games/shared/GameSession.jsx';
import './GamePage.css';
...
export function GamePage({ game }) {
  ...
  const title = game.key === 'snake' ? game.title : copy.games.titles[game.key] ?? game.title;
  const playable = ['snake', 'dino', 'game-of-life'].includes(game.key);
  const content = (
  ...
  return (
    <>
      ...
      <main className={`game-page${playable ? ' game-page--playable' : ''}`} dir="ltr">
        {!playable && <h1>{title}</h1>}
        {playable ? (
          <GameSession key={game.key} gameKey={game.key} title={title}>
            {content}
          </GameSession>
        ) : content}
      </main>
